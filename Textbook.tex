...
\texttt{newInventory.sort\_index(inplace=True, ascending=False)}
...
\texttt{newInventory.sort\_values("Price", inplace=True, ascending=False)}

\section{Arithmetic operations}

Pandas also provides support for the four basic arithmetic operations (addition, subtraction, multiplication, and division). Let's say that we want to take the \texttt{onHand} and \texttt{Price} columns from the \texttt{newInventory} data frame, multiply the two, and create a new column that represents the extended price. 

\texttt{newInventory["Ext"] = newInventory.Price * newInventory.onHand}
...
\texttt{sumInventory = newInventory.groupby("SKU").sum()}

Printing the \texttt{sumInventory} data frame would yield the output presented in Figure 6.5.
...
\section{Discussion Questions}
\begin{enumerate}
	\item Manually create a data frame
	\item Re-index the data frame
	\item Sort the data frame in ascending order.
\end{enumerate}
\section{Exercises}
\chapter{Basic statistical analysis}
\chapter{Visualization using matlib}
\end{document}