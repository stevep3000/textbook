...
\texttt{import csv} \\
\texttt{output = open("inventory.csv", "a")}
...
The full source code for the program and the related CSV file (as viewed in Microsoft Excel) are presented in Figures 5.11 and 5.12, respectively.

\begin{figure}[h]
\caption{The complete basic web scraper}
\begin{lstlisting}
	import requests
...
Note that the program we developed

\section{Parsing XML}

\section{Discussion questions}
\begin{enumerate}
	\item Provide some basic tags and indicate what these are.
\end{enumerate}
\end{document}