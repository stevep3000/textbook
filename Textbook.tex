...
\section{Exercises}
\begin{enumerate}
	\item Manually create a data frame that contains a list of the five most recent Super Bowl champions (this information can easily be found via the web). The data frame should contain columns which list the team name, the year the title was won, and the city the game was played at. Print the frame to screen to ensure that it was constructed properly.
	\item Modify the data frame you prepared for question 1 so that the row index labels reflects the year the championship was won. Again, print the frame to screen to ensure that this change was properly made.
	\item Modify the data frame you prepared in questions 1 and 2 by adding an addition column called \texttt{Score} which reflects the final score of the game. Again, print the frame to screen to ensure that the new addition is appropriately included.
	\item \textbf{Portfolio project}: Perform an analysis to see if there is a relationship between per capita alcohol consumption and average temperature. You are interested in understanding whether people, on average, consume more alcohol in climates that are cold. You should follow these steps:
\begin{itemize}
